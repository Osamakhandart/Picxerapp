...
import 'package:fiberchat/Services/helpers/misc.dart';
...
Future<void> exportVideo() async {
  try {
    // Configure FFmpeg for video editing
    ...
    // Start exporting using FFmpeg command
    _isExporting.value = true;
    await FFmpegKit.executeAsync(execute.command, (session) async {
      ...
      if (ReturnCode.isSuccess(returnCode)) {
        ...
        if (_controller.selectedCoverVal != null) {
          // Use selected cover if available
          final Uint8List? thumbData = _controller.selectedCoverVal!.thumbData;
          if (thumbData != null) {
            final tempDir = await getTemporaryDirectory();
            thumbnailFile = await File(
                    '${tempDir.path}/${DateTime.now().millisecondsSinceEpoch}.png')
                .create();
            await thumbnailFile.writeAsBytes(thumbData);
          }
        } else {
          // Default thumbnail extraction using `video_thumbnail`
        ...
        // Pass the video and thumbnail to the callback
        Navigator.of(context).pop();
        widget.onEditExported(File(outputPath), thumbnailFile!);
      } else {
              ...
              Expanded(
                child: SplashTap(
                  onTap: () {
                    ...
                    //WAY 1:
                    // controller.updateCrop( controller.cacheMinCrop,);
                    /*WAY 2:
                    controller.minCrop = controller.cacheMinCrop;
                    controller.maxCrop = controller.cacheMaxCrop;
                    */
                    //change done
                    controller.cacheMinCrop = controller.cacheMinCrop;
                    controller.cacheMaxCrop = controller.cacheMaxCrop;
                    Navigator.of(context).pop();
                  },
                  child: Center(
                    child: Text(
