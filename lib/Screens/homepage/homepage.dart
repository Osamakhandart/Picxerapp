...
class HomepageState extends State<Homepage>
  ...
  void setIsActive() async {
    print('setIsActive running (this is only to test logging)');
  ...
  incrementSessionCount(String myphone) async {
    ...
    if (_sharedFiles!.length > 0 || _sharedText != null) {
      print("triggerSharing is called");
      triggerSharing();
    }
  }

  triggerSharing() {
    print("triggerSharing running");
...
/* NOT NEEDED probably because also my old plugin supported sharing to iOS
  //added by JH:
  List<SharedMediaFile> convertSharedFilesToSharedMediaFiles(
      List<SharedFile> sharedFiles) {
    return sharedFiles.map((sharedFile) {
      var sharedMediaFile = SharedMediaFile(
          sharedFile.value!, // Set path
          sharedFile.thumbnail, // Set thumbnail
          sharedFile.duration, // Set duration
          mapSharedMediaType(sharedFile.type) // Map and set type
          );
...
//added by JH:
  SharedMediaType mapSharedMediaType(SharedMediaType fromType) {
    switch (fromType) {
      case SharedMediaType.TEXT:
      case SharedMediaType.URL:
      case SharedMediaType.OTHER:
        return SharedMediaType.FILE; // Mapping to FILE as a fallback
      ...
      default:
        return SharedMediaType.FILE; // Fallback for any unspecified types
    }
  }*/

  listenToSharingintent() {
    print("listenToSharingintent running1");

    //if (Platform.isAndroid) {NOT NEEDED probably because also my old plugin supported this
    //this sadly didn't work for iOS so I splitted it up
...
_intentDataStreamSubscription =ReceiveSharingIntent.instance.getMediaStream()
  ...
  print("Media stream received");
  setState(() {
    _sharedFiles = value;
  });
  if (_sharedFiles!.isNotEmpty || _sharedText != null) {
    print("Triggering sharing logic...");
    triggerSharing();
  }
}, onError: (err) {
...
ReceiveSharingIntent.instance.getInitialMedia().then((List<SharedMediaFile> value) {
  ...
  debugPrint("Error receiving initial media: $err");
});

// Handle text sharing using media stream methods (if applicable)
_intentDataStreamSubscription = ReceiveSharingIntent.instance.getMediaStream()
    .listen((List<SharedMediaFile> value) {
  print("Text/URL stream received via media stream");
  setState(() {
    if (value.isNotEmpty) {
      // Extract text content if needed
      _sharedText = value.first.message; // Assuming content holds the text
    }
  });
}, onError: (err) {
  debugPrint("Error receiving text/URL stream: $err");
});

// Handle initial text shared with the app
ReceiveSharingIntent.instance.getInitialMedia().then((List<SharedMediaFile> value) {
  print("Initial text/URL received via media");
  setState(() {
    if (value.isNotEmpty) {
      // Extract text content if needed
      _sharedText = value.first.message; // Assuming content holds the text
  ...
  debugPrint("Error receiving initial text/URL: $err");
});

    /*} else if (Platform.isIOS) { NOT NEEDED probably because also my old plugin supported this
      // For sharing images coming from outside the app while the app is in the memory
                                        ...
                                        onSelected: (dynamic val) async {
                                          switch (val) {
                                            case 'rate':
                                              break;
                                              case 'donnate':
                                               DonationPopup.showDonationPopup(context,alwaysShow: true);
                                              break;
                                            case 'tutorials':
                                              showDialog(
                                                  context: context,
